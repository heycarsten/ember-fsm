...
export default Ember.Object.extend({
  ...
  checkGuard: function(args, guardProperty, isInverse) {
    var target     = this.get('target');
  ...
  transitionFor: function(event, args) {
    ...
    for (i = 0; i < potentials.length; i++) {
      ...
      if (potential.doIf && this.checkGuard(args, potential.doIf)) {
        transitionParams = potential;
        break;
      }

      if (potential.doUnless && this.checkGuard(args, potential.doUnless, true)) {
        transitionParams = potential;
        break;
      }
    }
    ...
    transitionParams.target    = target;
