...
module('Unit: ember-fsm/-machine - create');
...
module('Unit: ember-fsm/-machine - transitionFor');
...
module('Unit: ember-fsm/-machine - inState');
...
module('Unit: ember-fsm/-machine - isIn{{stateName}} accessors');
...
module('Unit: ember-fsm/-machine - canEnterState');
...
module('Unit: ember-fsm/-machine - send', {
  afterEach() {
    stopIgnoringRejections();
  },
  ...
  beforeEach() {
    ...
    this.shutDown = sinon.spy(function() {
      return Ember.RSVP.resolve('shut down');
    });

    this.throwError = sinon.spy(function() {
      return Ember.RSVP.reject('exploded');
    });
    ...
    this.target = Ember.Object.create({
...
module('Unit: ember-fsm/-machine - transition activation', {
  beforeEach() {
    startIgnoringRejections();

    let runner = this;

    this.beforePromise = new Ember.RSVP.Promise((resolve) => {
      this.beforeResolver = resolve;
    });

    this.enterPromise = new Ember.RSVP.Promise((resolve) => {
      this.enterResolver = resolve;
    });

    this.afterPromise = new Ember.RSVP.Promise((resolve) => {
...
module('Unit: ember-fsm/-machine - send (while active)', {
  beforeEach() {
    this.fsm = createMachine({
      states: {
        initialState: 'one',
